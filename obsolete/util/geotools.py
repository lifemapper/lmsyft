"""Miscellaneous tools for reading and writing geospatial files."""
import glob
import os
...
def _create_empty_dataset(
    ...
    try:
        ...
        if dataset is None:
            raise Exception(f"Dataset creation failed for {out_shp_filename}")

        lyr = dataset.CreateLayer(
            dataset.GetName(), geom_type=ogr_type, srs=target_srs)
        if lyr is None:
            raise Exception(f"Layer creation failed for {out_shp_filename}.")
        ...
        for (fldname, fldtype) in feature_attributes:
            ...
            if return_val != 0:
                raise Exception(
                    f"CreateField failed for {fldname} in {out_shp_filename}")
        print(f"Created empty dataset with {len(feature_attributes)} fields")
    except Exception as e:
        print(f"Failed to create shapefile {out_shp_filename}", e)
...
def get_clustered_spatial_index(shp_filename):
    ...
    Args:
        shp_filename: full filename of a shapefile for creating an index.
...
def intersect_write_shapefile(new_dataset, new_layer, feats, grid_index):
    ...
    Args:
        new_dataset: new OGR Data Source for writing intersecting features.
        new_layer: new OGR Layer for writing intersecting features
        feats: ogr features to intersect with the spatial index
        grid_index: rtree spatial index
    """
    feat_count = 0
    # for each feature
...
def simplify_merge_polygon_shapefiles(in_shp_filenames, calc_fields, out_shp_filename):
    ...
    Args:
        ...
        calc_fields: calculated fields to add to features.
...
def intersect_polygon_with_grid(
        primary_shp_filename, grid_shp_filename, calc_fields, out_shp_filename):
    """Intersect a primary shapefile with a simple polygon, simplifying multipolygons.

    Intersect the simple polygons with gridcells in the second shapefile to further
